...
local actions,released_velocity,release_behavior
...
local pc={RequestEndInteraction=function(_,controller)
    ...
    if controller==right then
        if release_behavior=='reject' then return false end
        if release_behavior=='error' then error('native release failed') end
        if held and held.ProjectileMovement then
            local velocity={X=0,Y=0,Z=0}
            actions.apply_throw_velocity(controller,velocity)
            released_velocity={X=velocity.X*1.6,Y=velocity.Y*1.6,Z=velocity.Z*1.6}
        end
    end
...
actions=dofile('ItemActions.lua')
...
now=.10; local x,y,z,cut=actions.slash(state)
check(cut and x>0 and y<0 and z<0,'melee crosses the view diagonally during its short cut')
...
actions.tick(state,false,true,pc,left,right,camera)
check(not held and released_velocity.X==1400 and released_velocity.Z==180 and state.message=='grenade-thrown',
    'native release receives camera-directed velocity before building the network drop request')
check(grenade.ProjectileMovement.Velocity==nil,'throw does not create a separate local projectile velocity')
local untouched={X=1,Y=2,Z=3}; actions.apply_throw_velocity(right,untouched)
check(untouched.X==1 and untouched.Z==3,'normal swing and later drops keep stock velocity')
local previous=releases; actions.tick(state,false,true,pc,left,right,camera)
check(releases==previous,'throw is not repeated')
held=grenade; actions.press(state,held,pc,left,right); release_behavior='reject'
actions.tick(state,false,true,pc,left,right,camera)
check(held==grenade and state.message=='throw-rejected','rejected native release keeps the grenade')
release_behavior='error'; actions.press(state,held,pc,left,right)
check(not pcall(actions.tick,state,false,true,pc,left,right,camera),'native release errors still reach the control cleanup')
actions.apply_throw_velocity(right,untouched)
check(untouched.X==1 and untouched.Z==3,'release errors cannot leave synthetic swing velocity enabled')
release_behavior=nil; actions.stop(state,pc,left,right)
