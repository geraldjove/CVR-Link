...
local stock_class=object({GetSuperStruct=function() return nil end})
local pawn=object({InputMode=0,PlayerVest=object({GetClass=function() scans=scans+1;return stock_class end})})
local pc=object({IsInputKeyDown=function() return false end})
local holders={}
FindAllOf=function() error('menu must use the local vest') end
...
check(tick() and scans==1,'new allowed world scans all three holders once')
for i=1,100 do now=10+i/120;assert(tick()) end
check(scans==1,'missing menus do not rescan every frame')
...
game.address=101
check(tick() and scans==baseline+1,'world changes retry immediately despite the previous backoff')
...
local holder_class=object({name='BlueprintGeneratedClass /CVRFlatscreen/BP_CVRHolder0.BP_CVRHolder0_C',GetSuperStruct=function() return nil end})
holder.GetClass=function() scans=scans+1;return holder_class end
pawn.PlayerVest=holder;now=now+.5;tick()
check(menu.hud()==nil and scans==baseline+1,'late replication waits only for the bounded retry')
now=now+.51;tick()
check(menu.hud()==widget.FlatHUD and scans==baseline+2,'late local widget is discovered and validated on retry')
for i=1,200 do now=now+.01;assert(tick()) end
check(scans==baseline+2,'a valid menu uses its existing component without global scans')
